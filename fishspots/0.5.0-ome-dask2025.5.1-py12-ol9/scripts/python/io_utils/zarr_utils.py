...
def create_dataset(container_path, dataset_subpath, shape, chunks, dtype,
    ...
    try:
        ...
        if dataset_subpath and dataset_subpath != '.':
            ...
            print(f'Create root array {container_path} {dataset_attrs}')
        ...
        print(f'Error creating a dataset at {container_path}:{dataset_subpath}: {e}')
        raise e
...
def open(data_path, data_subpath, data_store_name=None,
    ...
    try:
        ...
        print(f'Open zarr container: {data_store}:{data_subpath} ({data_store_name})')
        ...
        if _is_ome_zarr(data_container_attrs):
            ...
            if block_coords is not None:
                print(f'Get block at {block_coords}')
                ba = a[block_coords]
            else:
                ba = a
...
def _get_array_selection(arr, axes, timeindex: int | None,
    ...
    if selection_exists:
        try:
            ...
            print(f'Get block at {block_slice_coords}')
            return arr[block_slice_coords]
        except Exception  as e:
            print(f'Error selecting data with selector {tuple(selector)}')
            raise e
    else:
        return arr
